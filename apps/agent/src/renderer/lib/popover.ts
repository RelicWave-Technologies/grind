...
export function usePopover(opts: PopoverOptions = {}): PopoverState {
  ...
  useLayoutEffect(() => {
    ...
    const flip: 'down' | 'up' = spaceBelow < estimatedHeight && r.top > estimatedHeight ? 'up' : 'down';
    const top = flip === 'down' ? r.bottom + gap : r.top - gap;
  ...
  const popoverStyle: React.CSSProperties = useMemo(() => {
    const base: React.CSSProperties = { left: pos.left };
    if (pos.flip === 'down') base.top = pos.top;
    else base.bottom = window.innerHeight - pos.top;
    return base;
  }, [pos.top, pos.left, pos.flip]);
