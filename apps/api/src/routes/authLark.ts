...
authLarkRouter.get('/start', (req, res) => {
  ...
  if (!isLarkConfigured()) return finish(res, client, { error: 'config' });
  const cfg = getLarkConfig();
  if (!cfg.redirectUri) return finish(res, client, { error: 'config' });

  let agentChallenge: string | undefined;
  if (client === 'agent') {
    agentChallenge = typeof req.query.code_challenge === 'string' ? req.query.code_challenge : '';
    if (!agentChallenge) return finish(res, client, { error: 'invalid_request' });
  }

  const nonce = crypto.randomBytes(16).toString('base64url');
  const state = signLoginState({ nonce, client, agentChallenge });
...
authLarkRouter.get('/callback', async (req, res, next) => {
  ...
  let payload: { nonce: string; client: 'dashboard' | 'agent'; agentChallenge?: string };
  try {
  ...
  try {
    if (req.query.error) return finish(res, client, { error: 'denied' });
    if (!req.query.code) return finish(res, client, { error: 'invalid_request' });
    if (!isLarkConfigured()) return finish(res, client, { error: 'config' });
    ...
    if (client === 'dashboard') {
      ...
      if (!cookieNonce || cookieNonce !== payload.nonce) {
        return finish(res, client, { error: 'state_invalid' });
      }
    }
    ...
    const cfg = getLarkConfig();
    if (!tm || !pc || !cfg.redirectUri) return finish(res, client, { error: 'config' });
    ...
    try {
      ...
      if (err instanceof LarkTransientError) return finish(res, client, { error: 'temporary' });
      logger.warn({ err: String(err), nonce: payload.nonce }, 'lark login: code exchange failed');
      return finish(res, client, { error: 'auth_failed' });
    }

    // 2. Fetch profile.
    const profile = await pc.getProfile(tokens.accessToken);
    if (!profile) return finish(res, client, { error: 'auth_failed' });
    if (!profile.email) return finish(res, client, { error: 'no_email' });
    ...
    if (user.deactivatedAt) return finish(res, client, { error: 'deactivated' });
    if (user.provisioningStatus !== 'ACTIVE') return finish(res, client, { status: 'pending' });
