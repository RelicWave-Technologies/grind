 ...
 * in as the primary ADMIN (abhishek@emiactech.com):
 *
 *   - ADMIN `abhishek@emiactech.com` → LarkIdentity reverted to his real
 ...
 *
 * After this script, abhishek (ADMIN) submitting a manual-time request from
...
const PRIMARY_ADMIN_REAL_OPENID = 'ou_51cd8f031e932fdabb7c73a51ffcc1c1'; // "Abhishek Verma" — the actual Lark account behind that email
...
async function main() {
  ...
  // 1) Promote test-requester → ADMIN (kept in the same workspace as the primary admin).
  if (admin.role !== 'ADMIN') {
  ...
  // 3) Restore the primary admin's real Lark identity. Detach any other holder of that open_id first.
  await prisma.larkIdentity.updateMany({
    where: { openId: PRIMARY_ADMIN_REAL_OPENID, userId: { not: primaryAdmin.id } },
    data: { openId: `__stash_${Date.now()}_primary_admin` },
  });
  await prisma.larkIdentity.upsert({
    where: { userId: primaryAdmin.id },
    create: { userId: primaryAdmin.id, openId: PRIMARY_ADMIN_REAL_OPENID },
    update: { openId: PRIMARY_ADMIN_REAL_OPENID },
  });
  console.log(`${PRIMARY_ADMIN_EMAIL} LarkIdentity → ${PRIMARY_ADMIN_REAL_OPENID} (Abhishek Verma)`);

  // Final summary.
  const final = await prisma.user.findMany({
    where: { workspaceId: primaryAdmin.workspaceId },
